"""
LLM-as-a-Judge Evaluation Script for CoT Mitigation

Evaluates model responses using parsed_answer (after [[answer]]) instead of full model_response.
...
async def judge_indirect_with_policy(
    ...
    Uses parsed_answer instead of model_response
    """
    parsed_answer = instance.get("parsed_answer", "")

    # Leakage evaluation
    leakage_user_prompt = JUDGE_INDIRECT_LEAKAGE_USER.format(
        policy=instance.get("policy", ""),
        policy_value=instance.get("policy_value", ""),
        model_response=parsed_answer,  # Use parsed_answer
    )
...
async def process_instance(
    ...
    """Process a single instance based on its type (with_policy only for CoT)."""
    async with sem:
...
def print_summary(items: List[Dict[str, Any]], output_dir: Path, metrics_filename: str = "summary_metrics.json") -> None:
    ...
    print("\n" + "="*60)
    print("EVALUATION SUMMARY (CoT Mitigation)")
    print("="*60)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="LLM-as-a-Judge evaluation for CoT mitigation responses (uses parsed_answer)."
    )
    parser.add_argument(
        "--input-file",
        type=str,
        required=True,
        help="Path to input JSON file (CoT evaluation output from 04_mitigation_cot.py).",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    for k, v in vars(args).items():
        print(f"  {k}: {v}")
    print("Mitigation: CoT (Chain of Thought) - using parsed_answer for evaluation")
