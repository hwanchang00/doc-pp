...
# CoT User Prompt Template
COT_USER_PROMPT_TEMPLATE = """{query}
...
def formulate_system_prompt(instance: Dict[str, Any]) -> str:
    """
    Build the system prompt from the given instance (with_policy only).
    """
...
def formulate_user_prompt_cot(instance: Dict[str, Any]) -> str:
    """
    Build the CoT user prompt from the given instance.
    """
    query = instance.get("query", "")
    return COT_USER_PROMPT_TEMPLATE.format(query=query)


def parse_cot_answer(model_response: str) -> str:
    """
    Extract everything after the [[answer]] marker from a CoT response.
    """
    # Locate the [[answer]] marker
    pattern = r'\[\[answer\]\]'
    match = re.search(pattern, model_response, re.IGNORECASE)

    if match:
        # Grab the substring after the marker
        answer_start = match.end()
        answer = model_response[answer_start:].strip()
        return answer
    else:
        # No marker: fall back to the entire response
...
async def process_instance(
    ...
    """Process a single instance using the CoT mitigation pipeline."""
    async with sem:
        doc_id = instance.get("doc_id", "")
        if not doc_id:
            print(f"[WARN] instance missing doc_id: {instance}")
            return {**instance, "model_response": "", "parsed_answer": "", "error": "Missing doc_id"}
        ...
        # Build the CoT prompts
        system_prompt = formulate_system_prompt(instance)
        user_prompt_text = formulate_user_prompt_cot(instance)
        ...
        parsed_answer = parse_cot_answer(model_response)

        print(" done", flush=True)
        return {
            **instance,
            "doc_mode": doc_mode,
            "with_policy": True,  # CoT mitigation always includes policy
...
async def main_async(args: argparse.Namespace) -> None:
    """Async entry point for the CoT mitigation pipeline."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate model with CoT (Chain of Thought) mitigation strategy."
    )
    parser.add_argument(
        "--input-file",
        type=str,
    ...
    parser.add_argument(
        "--output-file",
        type=str,
        default="evaluation_cot_output.json",
    ...
    for k, v in vars(args).items():
        print(f"  {k}: {v}")
    print(f"Mitigation: CoT (Chain of Thought)")
