"""
LLM-as-a-Judge Evaluation Script for Revision Mitigation

Evaluates model responses using revised_response instead of full model_response.
...
async def judge_direct_with_policy(
    ...
    Uses revised_response instead of model_response
    """
    user_prompt = JUDGE_DIRECT_WITH_POLICY_USER.format(
        policy=instance.get("policy", ""),
        policy_value=instance.get("policy_value", ""),
        model_response=instance.get("revised_response", ""),  # Use revised_response
    )
...
async def judge_indirect_with_policy(
    ...
    """
    Case 2: Indirect + With Policy -> Leakage + Faithfulness (separate API calls)
    Uses revised_response instead of model_response
    """
    revised_response = instance.get("revised_response", "")

    # Leakage evaluation
    leakage_user_prompt = JUDGE_INDIRECT_LEAKAGE_USER.format(
        policy=instance.get("policy", ""),
        policy_value=instance.get("policy_value", ""),
        model_response=revised_response,  # Use revised_response
    )
...
async def process_instance(
    ...
    """Process a single instance based on its type (with_policy only for Revision)."""
    async with sem:
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    # Load existing results and filter processed
    ...
    async def process_and_save(instance: Dict[str, Any]) -> Dict[str, Any]:
        """Process instance and save immediately."""
...
def print_summary(items: List[Dict[str, Any]], output_dir: Path, metrics_filename: str = "summary_metrics.json") -> None:
    ...
    print("\n" + "="*60)
    print("EVALUATION SUMMARY (Revision Mitigation)")
    print("="*60)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="LLM-as-a-Judge evaluation for Revision mitigation responses (uses revised_response)."
    )
    parser.add_argument(
        "--input-file",
        type=str,
        required=True,
        help="Path to input JSON file (Revision evaluation output from 04_mitigation_revision.py).",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--output-file",
        type=str,
        default="judge_revision_output.json",
    ...
    for k, v in vars(args).items():
        print(f"  {k}: {v}")
    print("Mitigation: Revision - using revised_response for evaluation")
